...
export const signup = asyncHandler(async (req,res, next) =>{
    ...
    const hashPassowrd = bcrypt.hashSync(password, parseInt(process.env.SALT_ROUND));
    const user = await userModel.create({userName, email, password: hashPassowrd, age, gender, phone });
    
    const token = jwt.sign({ email : user.email, id : user._id}, process.env.EMAIL_SIGNATURE, { expiresIn: 60 * 5 });
    const link = `${protocol}://${host}/auth/cofirm-email/${token}`;
