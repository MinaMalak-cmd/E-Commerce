...
async function createInvoice(invoice, pathVar) {
  ...
  doc.end()
//   doc.pipe(fs.createWriteStream(path.resolve(pathVar))) // this will download file on root directory
  doc.pipe(fs.createWriteStream(path.resolve(`./Files/${pathVar}`))) 
  // create -> to create the file
  // write -> you need to write on it
  // stream make changes happen in pdf in realtime
}
...
function generateHeader(doc) {
  doc
    ...
    // as long as I didn't add .fillColor('#09c')
    // .fontSize(10), then previous style will be applied
...
function generateInvoiceTable(doc, invoice) {
  ...
  for (i = 0; i < invoice.items.length; i++) {
    ...
    const position = invoiceTableTop + (i + 1) * 30 // 0
    generateTableRow(
      doc,
      position,
